...
async def send_register_query(username, userurl, user_id):
    ...
    builder.button(text='Дозволити', callback_data='user_registration_allow')
    builder.button(text='Заборонити', callback_data='user_registration_deny')
...
async def generate_rights_keyboard(review_3d, edit_3d,
    ...
    inline_keyboard.button(text=f"Перегляд 3д: "+ ("Та" if review_3d else "Ні"),
                           callback_data="review_3d")
    inline_keyboard.button(text=f"Редагування 3д: "+ ("Та" if edit_3d else "Ні"),
                           callback_data="edit_3d")
    inline_keyboard.button(text=f"Перегляд електроніки"+ ("Та" if review_electronics else "Ні"),
                           callback_data="review_electronics")
    inline_keyboard.button(text=f"Редагування електроніки: "+ ("Та" if edit_electronics else "Ні"),
...
async def send_3mf(message: types.Message, part_name, part_class: Union[Type[Parts3d]]):
    ...
    if three_mf:
        ...
        return False
    if old_three_mf:
        await message.answer("Старий .3mf:")
...
async def construct_part_info_keyboard(part_name, part_class: Union[Type[Parts3d]]):
    ...
    inline_keyboard.button(text=f'Імя: {part_info["name"]}', callback_data='name')
    inline_keyboard.button(text=f'Фото', callback_data='image')
    inline_keyboard.button(text=f'Кількість: x{part_info["count"]}', callback_data='count')
    inline_keyboard.button(text=f'Вага: {part_info["weight"]}', callback_data='weight')
    inline_keyboard.button(text=f'Час на А1mini: {part_info["A1mini"]}', callback_data='A1mini')
    inline_keyboard.button(text=f'Час на P1S: {part_info["P1S"]}', callback_data='P1S')
    inline_keyboard.button(text=f'Заповнення: {part_info["filling"]}', callback_data='filling')
